...
class BookRom extends React.Component {
    constructor (props) {
        super(props)
        ...
        this.state = {
            ...
            starttid: '',
            slutttid: '',
            formErrors: {email: '', navn: '', firma: '', phone: '', antall: '', room: '', dato: ''},
            emailValid: false,
            navnValid: false,
            firmaValid: false,
            phoneValid: false,
    ...
    validateField(fieldName, value) {
        ...
        switch(fieldName)
        {
            ...
            case 'antall':
                ...
                break;
            case 'room':
                roomValid = value.length >= 1;
                fieldValidationErrors.rom = roomValid ? '': 'is invalid';
                break;
            default:
                break;
        }
        this.setState({formErrors: fieldValidationErrors, navnValid: navnValid, firmaValid: firmaValid,
            emailValid: emailValid, phoneValid: phoneValid, antallValid: antallValid, roomValid: roomValid}, this.validateForm);
    }
                        ...
                        <Row>
                            <Col md="8" className="mt-3 mx-auto">
                                <Jumbotron>
                                    ...
                                    <br/><br/>
                                    <div className={`form-group ${this.errorClass(this.state.formErrors.room)}`}>
                                    <select name="room" id="room" type="text" required className="form-control"
                                            value={this.state.room}
                                            onChange={this.handleUserInput}>
                                        <option value="" disabled selected >Choose room</option>{
                                        this.state.roms.map((obj) => {
                                            return <option value={obj.id}>{obj.navn}</option>})}</select><br/>
                                    </div>
                                    <br/><br/>
                                    <div class="row">
                                        <div class="col-sm-4">
